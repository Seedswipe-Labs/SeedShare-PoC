...
contract SeedDAOVesting is SeedCore {

    ////////////////
    // EVENTS
    ////////////////

    event SeedCoreSet(address indexed dao, address[] accounts, uint256[] amounts, uint256[] startTimes, uint256[] endTimes);
    event SeedCoreCalled(address indexed dao, uint256 vestingId, address indexed member, uint256 indexed amountOut);
    ...
    function setSeedCore(
        ...
        virtual
    {
        (address[] memory accounts, uint256[] memory amounts, uint256[] memory startTimes, uint256[] memory endTimes) = abi.decode(SeedCoreData, (address[], uint256[], uint256[], uint256[]));
        ...
        emit SeedCoreSet(msg.sender, accounts, amounts, startTimes, endTimes);
    }
    ...
    function callSeedCore(
        ...
        uint256 vestingId = abi.decode(SeedCoreData, (uint256));
        ...
        emit SeedCoreCalled(msg.sender, vestingId, account, amount);
    }
}
